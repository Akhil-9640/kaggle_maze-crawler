...
class Strategy:
    ...
    def global_target_assignment(self, robots, danger):
        self.assigned_global_targets = {}

        # 1. Identify all units that need target assignment (non-factory, not forced, not special planned)
        assignable_units = []
        for r in robots:
            if r.rtype == FACTORY:
                continue
            if r.uid in self.forced_actions or r.uid in self.planned_special_actions:
                continue
            if r.energy <= self.energy_per_turn():
                continue
            assignable_units.append(r)

        if not assignable_units:
            return

        # 2. For each assignable unit, get their candidate targets
        unit_candidates = {}
        for r in assignable_units:
            candidates = select_candidate_targets(self.world, r, set())
            unit_candidates[r.uid] = candidates

        # 3. Create all matching pairs of (robot, target) and compute the utility
        bfs_cache = {}
        triples = []
        for r in assignable_units:
            assigned = self.world.state["assigned_targets"].get(r.uid)
            old_kind, old_pos, old_step = None, None, None
            ...
            for target in unit_candidates[r.uid]:
                cache_key = (r.uid, target.pos)
                if cache_key not in bfs_cache:
                    ...
                    bfs_cache[cache_key] = (step, dist)

                step, dist = bfs_cache[cache_key]
                if step is None or step == "IDLE":
                    continue

                period = move_period_for_robot(r, self.world.config)
                dist_penalty = 6.0 if r.rtype == SCOUT else 8.0
                utility = target.score - dist_penalty * dist * period

                # Hysteresis bonus if target matches previously assigned target
                if old_pos is not None and target.kind == old_kind and target.pos == old_pos:
                    if self.world.step - int(old_step) <= ASSIGNMENT_TTL:
                        utility += ASSIGNMENT_SWITCH_MARGIN

                triples.append((utility, r.uid, target))

        # 4. Global Greedy Match
        triples.sort(key=lambda x: x[0], reverse=True)

        assigned_robots = set()
        assigned_positions = set()

        for utility, r_uid, target in triples:
            if r_uid in assigned_robots or target.pos in assigned_positions:
                continue

            robot = self.world.own[r_uid]
            self.assigned_global_targets[r_uid] = target
            self.remember_target(robot, target)

            assigned_robots.add(r_uid)
            assigned_positions.add(target.pos)
    ...
    def plan(self):
        ...
        self.global_target_assignment(robots, danger)
    ...
    def unit_policy(self, robot):
        ...
        target = self.assigned_global_targets.get(robot.uid)
        if target is not None:
            if self.target_allowed_under_planned_ledger(robot, target):
                ...
                elif target.kind in {"wall_open", "factory_corridor_open", "worker_vanguard"} and robot.rtype == WORKER:
                ...
                elif target.pos == robot.pos and target.kind in {"mine_harvest", "crystal", "unload_factory"}:
                    self.remember_target(robot, target)
                    return "IDLE", robot.pos

                elif target.pos != robot.pos:
