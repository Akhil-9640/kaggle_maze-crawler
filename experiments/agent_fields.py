...
def build_dijkstra_map(world, robot, start_goals, max_nodes=650):
    goals = set(start_goals)
    if not goals:
        return {}
    ...
    while queue and nodes < max_nodes:
        ...
        for direction, nxt in world.neighbors(cur, allow_unknown_target=False):
            ...
            if nxt in world.own_positions and nxt not in goals:
                continue
...
class Strategy:
    ...
    def potential_field_policy(self, robot):
        candidates = [("IDLE", robot.pos)]
        for direction, nxt in self.world.safe_moves(robot, self.reserved_next, prefer_north=True):
            candidates.append((direction, nxt))

        if robot.rtype == WORKER:
            if self.wall_remove_legal(robot, "REMOVE_NORTH"):
                candidates.append(("REMOVE_NORTH", robot.pos))
        ...
        danger = self.dynamic_danger_gap()
        if robot.rtype == WORKER:
            danger_gap = robot.row - self.world.south
            allow_crystal = (
                robot.energy < WORKER_REFILL_ENERGY
                and danger_gap >= danger + WORKER_REFILL_SAFE_GAP
            )

        crystal_goals = []
        if allow_crystal:
            crystal_goals = list(self.world.visible_crystals.keys())

        frontier_goals = list(self.world.known_frontiers())

        unload_goals = []
        if robot.rtype == SCOUT and self.should_unload_scout(robot):
            unload_goals = self.world.unload_cells_for_factory()
        elif robot.rtype in (WORKER, MINER) and self.should_unload_non_scout(robot):
            unload_goals = self.world.unload_cells_for_factory()
            ...
            if cap and robot.energy < cap * UNLOAD_FRACTION:
                for mine_pos, mine_data in self.world.state["known_mines"].items():
                    ...
                    try:
                        ...
                        continue
                    if owner == self.world.player and energy >= MINE_HARVEST_MIN_ENERGY:
                        if node_lifetime_viable(self.world, mine_pos, min_turns=max(MINE_HARVEST_SCROLL_GAP, 4)):
                            mine_harvest_goals.append(mine_pos)

        node_goals = []
        if robot.rtype == MINER:
            for node in self.world.state["known_nodes"]:
                if node not in self.world.state["known_mines"] and node_lifetime_viable(self.world, node):
                    node_goals.append(node)

        worker_job_goals = []
        if robot.rtype == WORKER and self.world.factory is not None:
            f = self.world.factory
            corridor = [
                f.pos,
                (f.col, f.row + 1),
                (f.col - 1, f.row + 1),
            ...
            for cpos in corridor:
                if not self.world.in_bounds(cpos):
                    continue
                wall = self.world.wall_at(cpos)
                if wall is not None and ((wall & WALL_N) or (wall & WALL_S)):
                    worker_job_goals.append(cpos)

            factory_blocked = factory_path_blocked_walls(self.world, f)
            if factory_blocked:
                cells = sorted({item[0] for item in factory_blocked}, key=lambda p: manhattan(f.pos, p))
                worker_job_goals.append(cells[0])
            elif f.row - self.world.south <= danger + WORKER_VANGUARD_GAP:
                vanguard = (f.col, min(self.world.north, f.row + WORKER_VANGUARD_AHEAD))
                worker_job_goals.append(vanguard)

        crystal_map = build_dijkstra_map(self.world, robot, crystal_goals) if crystal_goals else {}
        frontier_map = build_dijkstra_map(self.world, robot, frontier_goals) if frontier_goals else {}
        unload_map = build_dijkstra_map(self.world, robot, unload_goals) if unload_goals else {}
        mine_harvest_map = build_dijkstra_map(self.world, robot, mine_harvest_goals) if mine_harvest_goals else {}
        node_map = build_dijkstra_map(self.world, robot, node_goals) if node_goals else {}
        worker_job_map = build_dijkstra_map(self.world, robot, worker_job_goals) if worker_job_goals else {}
        ...
        for action, pos in candidates:
            eval_pos = pos
            if action.startswith("REMOVE_"):
                eval_pos = step_pos(robot.pos, action.split("_", 1)[1])

            c_dist = crystal_map.get(eval_pos)
            c_pot = 0.0
            if c_dist is not None:
                energy = self.world.visible_crystals.get(eval_pos, 30)
                c_pot = (560.0 + CRYSTAL_VALUE_WEIGHT * energy) * (gamma ** c_dist)

            f_dist = frontier_map.get(eval_pos)
            f_pot = 0.0
            if f_dist is not None:
                f_pot = 300.0 * (gamma ** f_dist)

            un_dist = unload_map.get(eval_pos)
            un_pot = 0.0
            if un_dist is not None:
                un_pot = 1200.0 * (gamma ** un_dist)

            mh_dist = mine_harvest_map.get(eval_pos)
            mh_pot = 0.0
            if mh_dist is not None:
                mh_pot = 650.0 * (gamma ** mh_dist)

            nd_dist = node_map.get(eval_pos)
            nd_pot = 0.0
            if nd_dist is not None:
                nd_pot = 800.0 * (gamma ** nd_dist)

            wj_dist = worker_job_map.get(eval_pos)
            wj_pot = 0.0
            if wj_dist is not None:
                wj_pot = WORKER_VANGUARD_SCORE * (gamma ** wj_dist)

            y_gap = eval_pos[1] - self.world.south
            if y_gap <= danger:
                scroll_penalty = 5000.0 / max(0.1, y_gap)
            else:
                scroll_penalty = 100.0 / max(0.1, y_gap - danger)

            center_pull = -abs(eval_pos[0] - self.world.width // 2) * 2.0
            north_bias = (eval_pos[1] - robot.row) * 15.0

            score = 0.0
            if robot.rtype == SCOUT:
                score = 1.0 * c_pot + 0.5 * f_pot + 1.2 * un_pot + 0.8 * mh_pot - scroll_penalty + center_pull + north_bias
            elif robot.rtype == WORKER:
                score = 1.5 * wj_pot + 0.6 * c_pot + 0.3 * f_pot + 1.0 * un_pot + 0.5 * mh_pot - scroll_penalty + center_pull + north_bias
            elif robot.rtype == MINER:
                score = 1.8 * nd_pot + 0.5 * c_pot + 0.3 * f_pot + 1.0 * un_pot + 0.5 * mh_pot - scroll_penalty + center_pull + north_bias
            else:
                score = north_bias + center_pull - scroll_penalty

            if action == "IDLE" and len(candidates) > 1:
                score -= 10.0
    ...
    def unit_policy(self, robot):
        ...
        return self.potential_field_policy(robot)
