...
cover_image_path = Path("/kaggle/input/datasets/pilkwang/pilkwang-public-dataset-for-notebooks-figures/Maze_Main.png")
...
cover_image_path = Path("/kaggle/input/datasets/pilkwang/pilkwang-public-dataset-for-notebooks-figures/Maze_Fig3.png")
if cover_image_path.exists():
    display(Image(filename=str(cover_image_path)))
...
display(HTML(r"""
<div style="max-width:100%;margin:0 auto;padding:18px 6px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#233044;">
  <div style="background:#f7fafc;border:1px solid #d8e4f1;border-radius:16px;padding:24px 28px;box-shadow:0 10px 28px rgba(31,45,66,.06);overflow:visible;">
    <div style="font-size:34px;font-weight:800;margin-bottom:8px;">🧭 A Control Problem, Not A Prediction Problem</div>
    <div style="font-size:17px;line-height:1.55;color:#506176;max-width:1180px;margin-bottom:20px;">
      The policy protects the factory first, turns partial observations into memory, uses BFS only over trusted map facts, and reserves cells before actions are returned.
    </div>
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px;align-items:stretch;">
      <div style="border:1px solid #cad8e8;background:white;border-radius:8px;padding:14px;min-width:0;overflow-wrap:break-word;">
        <div style="font-size:26px;">🌊</div><div style="font-weight:800;font-size:14px;line-height:1.25;white-space:normal;">Moving Loss Boundary</div>
        <div style="font-size:13px;color:#66778c;line-height:1.45;">Factory margin is a constraint before it is a score feature.</div>
      </div>
      <div style="border:1px solid #cad8e8;background:white;border-radius:8px;padding:14px;min-width:0;overflow-wrap:break-word;">
        <div style="font-size:26px;">🌫️</div><div style="font-weight:800;font-size:14px;line-height:1.25;white-space:normal;">Partial Observation</div>
        <div style="font-size:13px;color:#66778c;line-height:1.45;">The agent must decide which facts persist after fog hides them.</div>
      </div>
      <div style="border:1px solid #cad8e8;background:white;border-radius:8px;padding:14px;min-width:0;overflow-wrap:break-word;">
        <div style="font-size:26px;">💥</div><div style="font-weight:800;font-size:14px;line-height:1.25;white-space:normal;">Action Interference</div>
        <div style="font-size:13px;color:#66778c;line-height:1.45;">Friendly robots can destroy each other without a reservation system.</div>
      </div>
      <div style="border:1px solid #cad8e8;background:white;border-radius:8px;padding:14px;min-width:0;overflow-wrap:break-word;">
        <div style="font-size:26px;">⛏️</div><div style="font-weight:800;font-size:14px;line-height:1.25;white-space:normal;">Delayed Economy</div>
        <div style="font-size:13px;color:#66778c;line-height:1.45;">Scouts and mines are valuable only when travel, scroll, and safety allow payback.</div>
      </div>
    </div>
  </div>
</div>
...
fig5_paths = [
    Path("/kaggle/input/datasets/pilkwang/pilkwang-public-dataset-for-notebooks-figures/Maze_Fig5.png"),
    Path("Figures/Maze_Fig5.png"),
]
for fig5_path in fig5_paths:
    if fig5_path.exists():
...
fig6_paths = [
    Path("/kaggle/input/datasets/pilkwang/pilkwang-public-dataset-for-notebooks-figures/Maze_Fig6.png"),
    Path("Figures/Maze_Fig6.png"),
]
for fig6_path in fig6_paths:
    if fig6_path.exists():
...
fig7_paths = [
    Path("/kaggle/input/datasets/pilkwang/pilkwang-public-dataset-for-notebooks-figures/Maze_Fig7.png"),
    Path("Figures/Maze_Fig7.png"),
]
for fig7_path in fig7_paths:
    if fig7_path.exists():
...
fig8_paths = [
    Path("/kaggle/input/datasets/pilkwang/pilkwang-public-dataset-for-notebooks-figures/Maze_Fig8.png"),
    Path("Figures/Maze_Fig8.png"),
]
for fig8_path in fig8_paths:
    if fig8_path.exists():
...
fig9_paths = [
    Path("/kaggle/input/datasets/pilkwang/pilkwang-public-dataset-for-notebooks-figures/Maze_Fig9.png"),
    Path("Figures/Maze_Fig9.png"),
]
for fig9_path in fig9_paths:
    if fig9_path.exists():
        display(Image(filename=str(fig9_path)))
        break
...
assert Path('main.py').read_text() == Path('submission.py').read_text()

# Kaggle/Papermill may execute the notebook as __notebook__.ipynb or without
# the original notebook file in the working directory.  The submission itself
# should remain standalone, so this source-notebook check is optional at
# runtime.  When a matching notebook file is present locally, it catches stale
# module cells or a broken %%writefile / %%writefile -a order.
notebook_paths = sorted(Path('.').glob('maze-crawler-structured-baseline*.ipynb'))
valid_source_notebooks = []
for notebook_path in notebook_paths:
    ...
    writefile_cells = [
        ...
        and ''.join(cell.get('source', [])).lstrip().startswith('%%writefile')
    ]
    headers = [src.splitlines()[0].strip() for src in writefile_cells]
    ordered_headers = (
        len(headers) == 6
        and headers[0] == '%%writefile main.py'
        and all(header == '%%writefile -a main.py' for header in headers[1:])
    )
    combined_source = ''.join(src.partition(chr(10))[2] for src in writefile_cells)
    if ordered_headers and combined_source == Path('main.py').read_text():
        valid_source_notebooks.append(str(notebook_path))
if notebook_paths:
    assert valid_source_notebooks, 'notebook file is present, but no synced ordered %%writefile layout was found'
    source_status = 'OK'
else:
    source_status = 'skipped: source notebook file is not present in this runtime'

controller = importlib.import_module('main')

print('syntax: OK')
print('writefile layout:', source_status)
print('submission sync: OK')
print('agent callable:', callable(controller.agent))
...
def make_walls(open_default=0, north_wall_at=None, custom=None, south=0):
    arr = [open_default] * (CONFIG['width'] * CONFIG['height'])
    for col, row in north_wall_at or []:
        arr[(row - south) * CONFIG['width'] + col] |= controller.WALL_N
    for (col, row), bits in (custom or {}).items():
        arr[(row - south) * CONFIG['width'] + col] = bits
    return arr


def make_obs(robots, wall_arr=None, crystals=None, nodes=None, mines=None, step=0, south=0, north=19, player=0):
    return {
        'player': player,
        'step': step,
        'walls': wall_arr if wall_arr is not None else make_walls(south=south),
        'crystals': crystals or {},
        'robots': robots,
        'mines': mines or {},
        'miningNodes': nodes or {},
        'southBound': south,
        'northBound': north,
    }
...
def run_contract_checks():
    ...
    assert controller.mirror_pos((3, 7), CONFIG['width']) == (16, 7)
    assert controller.mirror_wall_bits(controller.WALL_N | controller.WALL_E) == (controller.WALL_N | controller.WALL_W)


    sparse_world = controller.WorldModel(
        make_obs(
            {},
            wall_arr=make_walls(open_default=-1, custom={(5, 5): controller.WALL_N}),
            step=0,
        ),
        CONFIG,
        state_store={},
    )
    checks.append(('reciprocal_sync_does_not_invent_unknown_cell', (5, 6) not in sparse_world.state['known_walls'], dict(sparse_world.state['known_walls'])))

    edge_world = controller.WorldModel(
        make_obs(
            {},
            wall_arr=make_walls(open_default=-1, custom={(5, 5): 0, (5, 6): controller.WALL_S}),
            step=0,
        ),
        CONFIG,
        state_store={},
    )
    checks.append(('movement_respects_opposite_known_wall', not edge_world.can_step((5, 5), 'NORTH', allow_unknown_target=True), None))


    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(make_obs({'f': [0, 5, 7, 1000, 0, 0, 0, 0]}), CONFIG)
    checks.append(('route_first_open_factory_moves_first', action['f'] == 'NORTH', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs(
            {'f': [0, 5, 7, 1000, 0, 0, 0, 0]},
            wall_arr=make_walls(custom={(5, 8): controller.WALL_N}),
            step=0,
        ),
        CONFIG,
    )
    checks.append(('opening_spawn_wall_moves_first', action['f'] == 'NORTH', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs({'f': [0, 5, 12, 1000, 0, 2, 10, 0]}, step=50, south=0),
        CONFIG,
    )
    checks.append(('route_exists_blocks_cooldown_build', action['f'] == 'IDLE', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs(
            {'f': [0, 5, 10, 1000, 0, 2, 10, 0]},
            wall_arr=make_walls(custom={
                (5, 10): controller.WALL_E | controller.WALL_W | controller.WALL_S,
                (5, 11): controller.WALL_N | controller.WALL_E | controller.WALL_W,
            }),
            step=50,
            south=0,
        ),
    ...
    checks.append(('blocked_long_route_allows_worker_build', action['f'] == 'BUILD_WORKER', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs(
            {
                'f': [0, 5, 7, 1000, 0, 2, 0, 5],
                'w': [2, 5, 8, 220, 0, 0, 0, 0],
            },
            crystals={'6,8': 50},
            step=0,
    ...
    checks.append(('worker_vanguard_beats_safe_crystal_under_pressure', action['w'] == 'NORTH', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs(
            {
                'f': [0, 5, 12, 1000, 0, 2, 0, 5],
                'w': [2, 5, 13, 120, 0, 0, 0, 0],
            },
            crystals={'6,13': 50},
            step=0,
            south=0,
    ...
    checks.append(('worker_refills_from_crystal_when_factory_safe', action['w'] == 'EAST', action))
    ...
    checks.append(('worker_with_energy_keeps_vanguard_over_crystal', action['w'] == 'NORTH', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs(
            {'f': [0, 5, 5, 1000, 0, 0, 2, 0]},
            wall_arr=make_walls(custom={
                (5, 5): controller.WALL_E | controller.WALL_W | controller.WALL_S,
                (5, 6): controller.WALL_N | controller.WALL_E | controller.WALL_W,
            }),
            step=50,
            south=0,
            north=22,
    ...
    checks.append(('future_jump_route_turns_first_step_into_progress', action['f'] == 'NORTH', action))

    world = controller.WorldModel(
        make_obs(
            {
                'f': [0, 5, 1, 900, 0, 0, 0, 0],
                's': [1, 5, 2, 50, 0, 0, 0, 0],
    ...
    checks.append(('world_safe_destination_rejects_friendly', not world.safe_destination(world.factory, (5, 2), set()), None))

    class IdleStrategy(controller.Strategy):
        def policy_action(self, robot):
            return 'IDLE', robot.pos

    injected_state = {}
    action = controller.compute_actions(
        make_obs({'f': [0, 5, 7, 1000, 0, 0, 0, 0]}),
        CONFIG,
        time.perf_counter(),
        state_store=injected_state,
        strategy_cls=IdleStrategy,
    )
    checks.append(('strategy_and_state_injection', action['f'] == 'IDLE' and 0 in injected_state, action))

    class InvalidStrategy(controller.Strategy):
        def policy_action(self, robot):
            if robot.rtype == controller.FACTORY:
                return 'JUMP_NORTH', (robot.col, robot.row + 2)
            if robot.rtype == controller.WORKER:
                return 'REMOVE_NORTH', robot.pos
            if robot.rtype == controller.SCOUT:
                return 'TRANSFER_NORTH', robot.pos
            return 'IDLE', robot.pos

    action = controller.compute_actions(
        make_obs(
            {
                'f': [0, 5, 18, 1000, 0, 0, 0, 0],
                'w': [2, 4, 5, 250, 0, 0, 0, 0],
                's': [1, 6, 5, 50, 0, 0, 0, 0],
            },
            step=20,
        ...
        time.perf_counter(),
        state_store={},
        strategy_cls=InvalidStrategy,
    )
    checks.append(('commit_normalizes_invalid_policy_actions', action == {'f': 'IDLE', 'w': 'IDLE', 's': 'IDLE'}, action))

    class UnsafeBuildStrategy(controller.Strategy):
        def policy_action(self, robot):
            if robot.rtype == controller.FACTORY:
                return 'BUILD_SCOUT', robot.pos
            return 'IDLE', robot.pos
    ...
    action = controller.compute_actions(
        make_obs({'f': [0, 5, 2, 1000, 0, 0, 10, 0]}, step=10, south=0),
        CONFIG,
        time.perf_counter(),
        state_store={},
        strategy_cls=UnsafeBuildStrategy,
    )
    checks.append(('commit_blocks_policy_build_in_danger', action['f'] == 'IDLE', action))

    action = controller.compute_actions(
        make_obs(
            {'f': [0, 5, 8, 1000, 0, 0, 10, 0]},
            wall_arr=make_walls(custom={
                (5, 8): 0,
                (5, 9): controller.WALL_N | controller.WALL_E | controller.WALL_W,
            }),
            step=20,
        ),
        CONFIG,
    ...
    checks.append(('commit_blocks_policy_build_into_trapped_spawn', action['f'] == 'IDLE', action))



    class BorderlineMoveStrategy(controller.Strategy):
        def policy_action(self, robot):
            if robot.rtype == controller.SCOUT:
                return 'NORTH', controller.step_pos(robot.pos, 'NORTH')
            return 'IDLE', robot.pos

    action = controller.compute_actions(
        make_obs({'s': [1, 5, 5, CONFIG['energyPerTurn'], 0, 0, 0, 0]}, step=20),
        CONFIG,
        time.perf_counter(),
        state_store={},
        strategy_cls=BorderlineMoveStrategy,
    )
    checks.append(('commit_blocks_move_with_only_upkeep_energy', action['s'] == 'IDLE', action))

    class BorderlineRemoveStrategy(controller.Strategy):
        def policy_action(self, robot):
            if robot.rtype == controller.WORKER:
                return 'REMOVE_NORTH', robot.pos
            return 'IDLE', robot.pos
    ...
    action = controller.compute_actions(
        make_obs(
            {'w': [2, 5, 5, CONFIG['wallRemoveCost'], 0, 0, 0, 0]},
            wall_arr=make_walls(north_wall_at=[(5, 5)]),
            step=20,
        ),
        CONFIG,
        time.perf_counter(),
        state_store={},
        strategy_cls=BorderlineRemoveStrategy,
    )
    checks.append(('commit_blocks_wall_remove_without_post_upkeep_cost', action['w'] == 'IDLE', action))

    class BorderlineTransformStrategy(controller.Strategy):
        def policy_action(self, robot):
            if robot.rtype == controller.MINER:
                return 'TRANSFORM', robot.pos
            return 'IDLE', robot.pos

    action = controller.compute_actions(
        make_obs(
            {'m': [3, 5, 10, CONFIG['transformCost'], 0, 0, 0, 0]},
            nodes={'5,10': 1},
            step=20,
        ),
        CONFIG,
        time.perf_counter(),
        state_store={},
        strategy_cls=BorderlineTransformStrategy,
    )
    checks.append(('commit_blocks_transform_without_post_upkeep_cost', action['m'] == 'IDLE', action))

    class BorderlineFactoryBuildStrategy(controller.Strategy):
        def policy_action(self, robot):
            if robot.rtype == controller.FACTORY:
                return 'BUILD_WORKER', robot.pos
            return 'IDLE', robot.pos
    ...
    action = controller.compute_actions(
        make_obs({'f': [0, 5, 8, CONFIG['workerCost'], 0, 0, 0, 0]}, step=20),
        CONFIG,
        time.perf_counter(),
        state_store={},
        strategy_cls=BorderlineFactoryBuildStrategy,
    )
    checks.append(('commit_blocks_factory_build_without_post_upkeep_cost', action['f'] == 'IDLE', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs({'w': [2, 5, 5, 250, 0, 0, 0, 0]}, make_walls(north_wall_at=[(5, 5)])),
        CONFIG,
    )
    checks.append(('worker_removes_north_wall', action['w'] == 'REMOVE_NORTH', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(make_obs({'m': [3, 5, 10, 200, 0, 0, 0, 0]}, nodes={'5,10': 1}), CONFIG)
    checks.append(('miner_transforms_on_viable_node', action['m'] == 'TRANSFORM', action))
    ...
    next_cells = []
    starts = {'s1': (5, 5), 's2': (7, 5)}
    for uid, pos in starts.items():
        act = action[uid]
        if act in controller.DIRS:
            next_cells.append(controller.step_pos(pos, act))
    checks.append(('no_duplicate_next_cell', len(next_cells) == len(set(next_cells)), action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(make_obs({'f': [0, 5, 2, 1000, 0, 2, 0, 0]}, step=10, south=0), CONFIG)
    checks.append(('danger_no_build', not action['f'].startswith('BUILD_'), action))
    checks.append(('jump_requires_move_readiness', action['f'] != 'JUMP_NORTH', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(make_obs({'f': [0, 5, 7, 1000, 0, 2, 0, 0]}, step=350, south=5), CONFIG)
    checks.append(('emergency_jump_can_ignore_move_cd', action['f'] == 'JUMP_NORTH', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs(
            {'f': [0, 5, 7, 1000, 0, 0, 0, 0]},
            wall_arr=make_walls(custom={(5, 7): controller.WALL_N}),
            step=40,
            south=0,
        ),
        CONFIG,
    )
    checks.append(('noncritical_north_wall_preserves_jump', action['f'] != 'JUMP_NORTH', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs(
            {'f': [0, 5, 2, 1000, 0, 0, 0, 0]},
            wall_arr=make_walls(custom={(5, 2): controller.WALL_N | controller.WALL_E | controller.WALL_S, (4, 2): controller.WALL_S}),
            step=0,
            south=0,
        ),
        CONFIG,
    )
    checks.append(('danger_north_wall_uses_safe_jump_over_sidestep', action['f'] == 'JUMP_NORTH', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs(
            {'f': [0, 5, 6, 1000, 0, 0, 0, 0]},
            wall_arr=make_walls(custom={
                (5, 5): controller.WALL_E | controller.WALL_W | controller.WALL_S,
                (5, 6): controller.WALL_N | controller.WALL_E | controller.WALL_W,
            }),
    ...
    checks.append(('caution_dead_end_uses_jump_fallback', action['f'] == 'JUMP_NORTH', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(make_obs({'f': [0, 5, 4, 1000, 0, 2, 10, 0]}, step=10, south=0), CONFIG)
    checks.append(('caution_gap_plus_one_no_build', not action['f'].startswith('BUILD_'), action))
    ...
    checks.append(('high_energy_scout_unloads_from_side_cell', action['s'] == 'TRANSFER_WEST', action))
    ...
    checks.append(('scout_unload_requires_threshold_or_cap', action['s'] != 'TRANSFER_SOUTH', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs({
            'f': [0, 5, 5, 1000, 0, 0, 0, 5],
            's': [1, 5, 6, 95, 0, 0, 0, 0],
        }),
        CONFIG,
    )
    checks.append(('routine_scout_keeps_moving_without_liquidity_need', action['s'] != 'TRANSFER_SOUTH', action))
    ...
    checks.append(('north_cell_scout_unload_guard', not (action['f'] == 'EAST' and action['s'] == 'TRANSFER_SOUTH'), action))
    ...
    checks.append(('transfer_threshold_uses_next_turn_energy', not controller.transfer_would_cross_build_threshold(world, world.own['s']), None))
    ...
    targets = controller.select_candidate_targets(world, world.own['s'], set())
    checks.append(('capped_scout_prefers_unload_over_frontier', targets[0].kind == 'unload_factory', [(t.kind, t.pos, round(t.score, 1)) for t in targets[:3]]))

    world = controller.WorldModel(
        make_obs(
            {
                'f': [0, 5, 12, 1000, 0, 2, 0, 0],
                's': [1, 8, 12, 0, 0, 0, 0, 0],
            },
            step=80,
            south=0,
    ...
    scout_target = controller.scout_target_count_for_world(world, world.factory)
    checks.append(('scout_target_requires_margin_surplus', scout_target == 0, scout_target))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(make_obs({'f': [0, 5, 16, 1000, 0, 2, 10, 0]}, step=50, south=0), CONFIG)
    checks.append(('margin_surplus_allows_one_cooldown_scout', action['f'] == 'BUILD_SCOUT', action))
    ...
    checks.append(('existing_scout_does_not_trigger_extra_build', action['f'] != 'BUILD_SCOUT', action))
    ...
    checks.append(('factory_build_suppresses_same_turn_liquidity_transfer', not (action['f'].startswith('BUILD_') and action['s'].startswith('TRANSFER_')), action))
    ...
    checks.append(('factory_build_suppresses_capped_scout_transfer', not (action['f'].startswith('BUILD_') and action['s'].startswith('TRANSFER_')), action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs({'m': [3, 5, 2, 200, 0, 0, 0, 0]}, nodes={'5,2': 1}, south=0),
        CONFIG,
    )
    checks.append(('low_lifetime_miner_no_transform', action['m'] != 'TRANSFORM', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs(
            {
                'f': [0, 5, 4, 300, 0, 2, 10, 5],
                'm': [3, 5, 5, 450, 0, 0, 0, 0],
            },
            nodes={'5,5': 1},
            step=0,
    ...
    checks.append(('early_lifetime_node_transform_beats_transfer', action['m'] == 'TRANSFORM', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs(
            {
                'f': [0, 5, 4, 1000, 0, 0, 10, 5],
                'm': [3, 5, 5, 450, 0, 0, 0, 0],
            },
            nodes={'5,5': 1},
            step=0,
    ...
    checks.append(('factory_enters_transforming_miner_cell', action['f'] == 'NORTH' and action['m'] == 'TRANSFORM', action))

    class NoTransformStrategy(controller.Strategy):
        def should_transform_miner(self, robot):
            return False
    ...
    action = controller.compute_actions(
        ...
        time.perf_counter(),
        state_store={},
        strategy_cls=NoTransformStrategy,
    )
    checks.append(('safety_does_not_force_heuristic_transform', action['m'] != 'TRANSFORM' and not (action['f'] == 'NORTH' and action['m'] in {'IDLE', 'TRANSFER_SOUTH'}), action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs(
            {
                'f': [0, 0, 5, 1000, 0, 0, 10, 5],
                'm': [3, 5, 5, 450, 0, 0, 0, 0],
                's': [1, 5, 4, 10, 0, 0, 0, 0],
            },
            nodes={'5,5': 1},
    ...
    checks.append(('scout_enters_newly_transformed_mine_cell', action['m'] == 'TRANSFORM' and action['s'] == 'NORTH', action))
    ...
    checks.append(('early_carrier_forces_transform_before_miner_turn', action['s'] == 'NORTH' and action['m'] == 'TRANSFORM', action))
    ...
    checks.append(('transformed_mine_harvest_beats_routine_scout_unload', action['m'] == 'TRANSFORM' and action['s'] == 'NORTH', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs(
            {
                'f': [0, 5, 11, 1000, 0, 0, 0, 0],
                'w': [2, 4, 11, 250, 0, 0, 0, 0],
                's': [1, 6, 11, 50, 0, 0, 0, 0],
            },
            nodes={'6,15': 1},
            step=50,
    ...
    checks.append(('simple_ferry_skips_remembered_node_miner', action['f'] != 'BUILD_MINER', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs(
            {
                'f': [0, 5, 15, 1000, 0, 0, 0, 0],
                'w': [2, 4, 15, 250, 0, 0, 0, 0],
                's1': [1, 6, 15, 50, 0, 0, 0, 0],
                's2': [1, 7, 15, 50, 0, 0, 0, 0],
                's3': [1, 8, 15, 50, 0, 0, 0, 0],
            },
            nodes={'10,18': 1},
            wall_arr=make_walls(custom={(10, 18): controller.WALL_N | controller.WALL_E | controller.WALL_S | controller.WALL_W}),
            step=50,
            south=0,
        ),
        CONFIG,
    )
    checks.append(('miner_build_requires_reachable_node', action['f'] != 'BUILD_MINER', action))
    ...
    checks.append(('skip_low_lifetime_node_miner', action['f'] != 'BUILD_MINER', action))
    ...
    checks.append(('immobile_blocker_not_assumed_vacated', not (action['f'] == 'NORTH' and action['w'] == 'IDLE'), action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs(
            {
                'f': [0, 5, 1, 900, 0, 0, 10, 0],
                'w': [2, 5, 2, 250, 0, 0, 0, 0],
            },
            step=10,
            south=0,
    ...
    checks.append(('vacate_possible_blocker_before_sacrifice', not (action['f'] == 'NORTH' and action['w'] in {'IDLE', 'TRANSFER_SOUTH'}), action))
    ...
    strategy = controller.Strategy(world, time.perf_counter(), time.perf_counter() + 1.0)
    before = (dict(strategy.forced_actions), dict(strategy.forced_reserved_next), set(strategy.reserved_next), set(strategy.sacrifice_uids))
    decision = strategy.evaluate_factory_destination(world.factory, (5, 5))
    after = (dict(strategy.forced_actions), dict(strategy.forced_reserved_next), set(strategy.reserved_next), set(strategy.sacrifice_uids))
    checks.append(('factory_destination_evaluation_is_pure', decision.ok and before == after, (decision.reason, before, after)))
    ...
    checks.append(('zero_energy_blocker_can_be_cleared', action['f'] == 'NORTH' and action['s'] == 'IDLE', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs(
            {
                'f': [0, 5, 7, 1000, 0, 2, 0, 0],
                's': [1, 4, 7, 50, 0, 0, 0, 0],
            },
            wall_arr=make_walls(custom={(5, 8): controller.WALL_N | controller.WALL_E | controller.WALL_W}),
        ),
        CONFIG,
    )
    checks.append(('side_route_beats_preemptive_worker_build', action['f'] == 'IDLE', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs({'s': [1, 5, 5, 20, 0, 0, 0, 0]}, mines={'5,7': [220, 1000, 0]}),
        CONFIG,
    )
    checks.append(('mine_harvest_target_moves_north', action['s'] == 'NORTH', action))
    ...
    action = controller.agent(
        make_obs(
            ...
            wall_arr=make_walls(custom={(5, 3): controller.WALL_N | controller.WALL_W}),
            crystals={'6,3': 40},
            step=10,
        ),
        CONFIG,
    )
    checks.append(('forced_vacate_destination_reserved', action['f'] == 'NORTH' and action['s'] == 'EAST' and action['w'] != 'NORTH', action))
    ...
    scout_next = controller.step_pos((5, 5), action['s']) if action['s'] in controller.DIRS else (5, 5)
    checks.append(('jump_landing_blocker_vacates', action['f'] == 'JUMP_NORTH' and scout_next != (5, 5), action))
    ...
    action = controller.agent(
        make_obs(
            {
                'f': [0, 5, 9, 1000, 0, 2, 10, 5],
                'm': [3, 5, 10, 400, 0, 0, 0, 0],
            },
            nodes={'5,10': 1},
            step=50,
        ),
    ...
    checks.append(('miner_transform_before_unload', action['m'] == 'TRANSFORM', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs(
            {
                'f': [0, 5, 5, 1000, 0, 2, 10, 5],
                'w': [2, 5, 6, 250, 0, 0, 0, 0],
            },
            wall_arr=make_walls(custom={(5, 5): controller.WALL_N, (5, 6): controller.WALL_S}),
            step=50,
        ),
        CONFIG,
    )
    checks.append(('worker_remove_south_for_factory_corridor', action['w'] == 'REMOVE_SOUTH', action))
    ...
    action = controller.agent(
        make_obs(
            ...
            wall_arr=make_walls(custom={(5, 5): controller.WALL_N}),
            step=50,
        ),
        CONFIG,
    )
    checks.append(('opposite_edge_wall_remove_from_worker_side', action['w'] == 'REMOVE_SOUTH', action))
    ...
    action = controller.agent(
        make_obs(
            ...
            wall_arr=make_walls(custom={(5, 6): controller.WALL_N}),
            step=50,
        ),
        CONFIG,
    )
    checks.append(('worker_corridor_forward_remove_north', action['w'] == 'REMOVE_NORTH', action))
    ...
    action = controller.agent(
        make_obs(
            ...
            wall_arr=make_walls(custom={(4, 5): controller.WALL_N}),
            step=50,
        ),
        CONFIG,
    )
    checks.append(('worker_wall_job_before_unload', action['w'] == 'REMOVE_NORTH', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs(
            {
                'f': [0, 5, 10, 800, 0, 0, 0, 0],
                'w': [2, 5, 11, 120, 0, 0, 0, 0],
            },
            wall_arr=make_walls(custom={(5, 11): controller.WALL_N}),
            step=0,
            south=0,
        ),
        CONFIG,
    )
    checks.append(('route_critical_worker_wall_job_can_be_refueled', action['f'] == 'TRANSFER_NORTH' and action['w'] == 'REMOVE_NORTH', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs(
            {
                'f': [0, 5, 12, 400, 0, 2, 10, 0],
                'w': [2, 5, 13, 120, 0, 0, 0, 0],
            },
            step=50,
            south=0,
    ...
    checks.append(('worker_without_wall_job_is_not_refueled', action['f'] != 'TRANSFER_NORTH', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs(
            {
                'f': [0, 5, 9, 1000, 0, 2, 10, 0],
                'w': [2, 5, 10, 120, 0, 0, 0, 0],
            },
            step=50,
            south=0,
        ),
    ...
    checks.append(('route_critical_worker_without_wall_job_no_blind_large_refuel', action['f'] != 'TRANSFER_NORTH', action))
    ...
    action = controller.agent(
        make_obs(
            ...
            wall_arr=make_walls(custom={(5, 13): controller.WALL_N}),
            step=50,
            south=0,
        ),
        CONFIG,
    )
    checks.append(('wall_job_worker_can_be_refueled', action['f'] == 'TRANSFER_NORTH' and action['w'] == 'REMOVE_NORTH', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs(
            {
                'f': [0, 5, 5, 1000, 0, 0, 0, 0],
                'w': [2, 5, 6, 150, 0, 0, 0, 0],
            },
            wall_arr=make_walls(custom={(5, 6): controller.WALL_N}, south=3),
            step=100,
            south=3,
            north=22,
        ),
    ...
    checks.append((
        'speculative_route_probe_does_not_vacate_wall_worker_before_jump',
        action['f'] == 'JUMP_NORTH' and action['w'] == 'REMOVE_NORTH',
        action,
    ))
    ...
    action = controller.agent(
        make_obs(
            {
                'f': [0, 5, 5, 1000, 0, 0, 5, 0],
                'w': [2, 5, 6, 150, 0, 0, 0, 0],
            },
            wall_arr=make_walls(custom={(5, 6): controller.WALL_N}, south=2),
            step=100,
            south=2,
            north=21,
        ),
    ...
    checks.append((
        'speculative_route_probe_does_not_vacate_wall_worker_before_refuel',
        action['f'] == 'TRANSFER_NORTH' and action['w'] == 'REMOVE_NORTH',
        action,
    ))
    ...
    checks.append(('non_scout_transfer_guard', action['w'] == 'REMOVE_NORTH', action))
    ...
    checks.append(('non_scout_north_corridor_transfer_guard', action['m'] != 'TRANSFER_SOUTH', action))

    world = controller.WorldModel(
        make_obs(
            {
                'f': [0, 5, 5, 700, 0, 2, 10, 5],
                'w': [2, 4, 5, 260, 0, 0, 0, 0],
                's1': [1, 3, 5, 20, 0, 0, 0, 0],
                's2': [1, 3, 6, 20, 0, 0, 0, 0],
            },
    ...
    worker = world.own['w']
    targets = controller.select_candidate_targets(world, worker, set())
    has_unload = any(target.kind == 'unload_factory' for target in targets)
    crosses = controller.transfer_would_cross_build_threshold(world, worker)
    checks.append(('non_scout_unload_target_requires_threshold_cross', not crosses and not has_unload, [(t.kind, t.pos, round(t.score, 1)) for t in targets[:4]]))
    ...
    checks.append(('peer_transfer_does_not_zero_scout', action['s'] != 'TRANSFER_EAST', action))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs(
            {
                'f': [0, 5, 8, 1000, 0, 2, 10, 5],
                'w': [2, 2, 8, 250, 0, 0, 0, 0],
            },
            wall_arr=make_walls(custom={(2, 8): controller.WALL_N}),
            crystals={'3,8': 40},
            step=80,
        ),
        CONFIG,
    )
    checks.append(('irrelevant_north_wall_not_preemptive', action['w'] != 'REMOVE_NORTH', action))

    controller.STATE_BY_PLAYER.clear()
    controller.agent(make_obs({'f': [0, 5, 5, 1000, 0, 2, 10, 5]}, nodes={'5,6': 1}, step=0), CONFIG)
    controller.agent(make_obs({'f': [0, 5, 5, 1000, 0, 2, 10, 5]}, mines={'5,6': [0, 1000, 0]}, step=1), CONFIG)
    checks.append(('mine_observation_clears_node_memory', (5, 6) not in controller.STATE_BY_PLAYER[0]['known_nodes'], dict(controller.STATE_BY_PLAYER[0]['known_nodes'])))
    ...
    strategy = controller.Strategy(world, time.perf_counter(), time.perf_counter() + 1.0)
    carrier_dist = strategy.nearest_harvest_carrier_distance(world.own['m'])
    checks.append(('carrier_reaches_occupied_miner_node', carrier_dist == 1, carrier_dist))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs(
            {
                'f': [0, 5, 8, 100, 0, 0, 10, 5],
                'w': [2, 5, 9, 250, 0, 0, 0, 0],
            },
            step=120,
        ),
    ...
    checks.append(('factory_follow_through_vacating_blocker', action['f'] == 'NORTH' and action['w'] == 'NORTH', action))

    mine_store = {}
    controller.WorldModel(
        make_obs(
            {'f': [0, 0, 2, 1000, 0, 0, 10, 5]},
            mines={'10,10': [0, 1000, 0]},
            step=0,
        ),
        CONFIG,
        state_store=mine_store,
    )
    world = controller.WorldModel(
        make_obs(
            {'f': [0, 0, 2, 1000, 0, 0, 10, 5]},
    ...
    estimate = controller.estimated_mine_energy(
        world,
        (10, 10),
        world.state['known_mines'][(10, 10)],
    )
    checks.append(('hidden_friendly_mine_energy_estimate', estimate == 250, estimate))


    state_store = {1: controller.fresh_state(1)}
    state_store[1]['step'] = 300
    state_store[1]['southBound'] = 200
    state_store[1]['known_walls'][(5, 250)] = 0
    world = controller.WorldModel(
        make_obs(
            {'1-f': [0, 15, 2, 1000, 1, 0, 0, 0]},
            step=None,
            south=0,
            player=1,
        ),
    ...
    checks.append(('player_one_initial_reset_without_uid_prefix', world.step == 0 and (5, 250) not in world.state['known_walls'], (world.step, (5, 250) in world.state['known_walls'])))

    state_store = {0: controller.fresh_state(0)}
    state_store[0]['step'] = 44
    state_store[0]['southBound'] = 6
    state_store[0]['known_walls'][(5, 9)] = 0
    world = controller.WorldModel(
        make_obs(
            {'f': [0, 5, 8, 950, 0, 0, 0, 0]},
            step=None,
    ...
    checks.append(('midgame_single_factory_with_south_does_not_reset', world.step == 45 and (5, 9) in world.state['known_walls'], (world.step, (5, 9) in world.state['known_walls'])))

    controller.STATE_BY_PLAYER.clear()
    action = controller.agent(
        make_obs(
            {
                'f': [0, 5, 15, 1000, 0, 0, 10, 0],
                'w': [2, 4, 15, 250, 0, 0, 0, 0],
            },
            step=430,
            south=8,
    ...
    checks.append(('late_game_scout_build_throttle', action['f'] != 'BUILD_SCOUT', action))

    controller.STATE_BY_PLAYER.clear()
    world = controller.WorldModel(
        make_obs({'s': [1, 5, 5, 20, 0, 0, 0, 0]}, crystals={'6,5': 30, '7,5': 35}, step=20),
        CONFIG,
        state_store={},
    )
    strategy = controller.Strategy(world, time.perf_counter(), time.perf_counter() + 1.0)
    candidates = [controller.Target('crystal', (7, 5), 500.0), controller.Target('crystal', (6, 5), 460.0)]
    world.state['assigned_targets']['s'] = ('crystal', (6, 5), world.step, 460.0)
    ordered = strategy.ordered_targets(world.own['s'], candidates)
    checks.append(('sticky_assignment_reorders_similar_target', ordered[0].pos == (6, 5), [t.pos for t in ordered]))
